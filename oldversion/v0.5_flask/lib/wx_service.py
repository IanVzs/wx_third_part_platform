...
class WeChat_OAP():
    def __init__(self, mode, appid=None):
        ...
        self.mini_app_ID = config.MINI_APP_ID
        self.url_unde_get_access_token = config.URL_GET_ACCESS_TOKEN
        self.url_unde_post_send_template_msg = config.URL_POST_SEND_TEMPLATE_MSG
        self.url_unde_get_template_list = config.URL_GET_TEMPLATE_LIST
        self.url_unde_get_openID_list = config.URL_GET_OPENID_LIST
        self.url_unde_get_user_s_info = config.URL_GET_USER_S_INFO
        self.url_unde_post_bind = config.URL_POST_BIND
        self.url_unde_post_create_menu = config.URL_POST_CREATE_MENU
        self.url_unde_get_get_menu = config.URL_GET_MENU
        self.url_unde_get_cur_self_menu = config.URL_GET_CUR_SELF_MENU
        self.url_unde_post_send_custom_msg = config.URL_SEND_CUSTOM
        self.url_unde_Get_This_Guy_qrcode = config.URL_GET_QRCODE_TICKET
        self.url_unde_get_jsapi_ticket = config.URL_GET_JSAPI_TICKET
    ...
    def format_url(self):
        ...
        self.url_Get_This_Guy_qrcode = self.url_unde_Get_This_Guy_qrcode.format(self.access_token)
    ...
    def do_get_cur_self_menu(self, url, test=False):
        ...
        log_info("get_cur_self_menu: inputs: url:{}, outputs: rsp_get_menu_msg: {}".format(
            self.url_get_menu, rsp_get_menu_msg.json()))
        errcode = rsp_get_menu_msg.json().get("errcode")
        if errcode:
            errcode = rsp_get_menu_msg.json().get("errcode")
            errmsg = rsp_get_menu_msg.json().get("errmsg")
    ...
    def do_Get_This_Guy_qrcode(self, url, scene_str):
        rsp_data = {"sign": 0, "msg": ''}
        ...
        data = {
            ...
            "action_info": {
                "scene": {
                    "scene_str": scene_str or "Get_This_Guy"
                }
            }
        }
    ...
    @check_access_token
    def Get_This_Guy_qrcode(self, scene_str):
        rsp_data = self.do_Get_This_Guy_qrcode(self.url_Get_This_Guy_qrcode, scene_str)
        return rsp_data
